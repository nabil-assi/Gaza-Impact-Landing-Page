...
export default function DonatePage() {
  // React Router: useSearchParams replaces TanStack's validateSearch + Route.useSearch()
          ...
          {method === "crypto" && (
            <div className="mt-6 rounded-xl border border-primary/30 bg-primary-soft/40 p-4 text-left text-sm text-moss">
              ...
              <p className="mt-1 text-foreground/80">
                Please send <strong>${amount}</strong> USDT (TRC-20) to the
                wallet below and contact us via WhatsApp with your transfer
                screenshot.
              </p>
              <code className="mt-2 block break-all rounded bg-white/70 px-3 py-2 font-mono text-xs text-foreground">
                {WALLET_ADDRESS}
              </code>
            </div>
          )}
  ...
  return (
    <PageShell>
      <div className="mx-auto max-w-6xl px-6 py-12">
        ...
        <button
          onClick={() =>
            navigate(initiativeId ? `/initiatives/${initiativeId}` : "/")
          }
          ...
          {!initiativeId && (
            <>
              ...
              <p className="mt-4 text-base text-muted-foreground">
                Your contribution goes to the General Humanitarian Fund,
                supporting wherever the need is greatest.
              </p>
            </>
          )}
